...
echo "$($PSQL "SELECT name FROM teams LEFT JOIN games AS winners ON teams.team_id = winners.winner_id LEFT JOIN games as opponent ON teams.team_id = opponent.opponent_id WHERE (winners.year = 2014 AND winners.round = 'Eighth-Final') OR (opponent.year = 2014 AND opponent.round = 'Eighth-Final') GROUP BY name")"
